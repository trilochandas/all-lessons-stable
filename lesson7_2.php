...
if (!file_exists('advert.txt')){
    fopen('advert.txt', 'w') or die("Unable to open file!");
}
    ...
    if (empty($_POST['title']) || empty($_POST['price']) ||  empty($_POST['seller_name']) || empty($_POST['phone'])) {
        echo '<p style="color:red;">Введите все данные</p>';
            ...
            if ( !is_null(file_get_contents('advert.txt'))) {
                ...
                $current = serialize($current);
                file_put_contents('advert.txt', $current);
            // если advert.txt пустой <- это фигня не срабатывает !!!!
            } else {
                // $advert=$_POST;
                $current = array();
                $current[] = $advert;
...
<form  method="POST">
    ...
    <?php
        ...
        foreach ($citys as $numb => $one_city) 
            echo '<option ' . $selected . ' value="' . $one_city['0'] . '" ' . ($location_id == $one_city['0']?"selected":"") .  '>' . $one_city['city'] . '</option>';
        echo '</select>';
    ?>
    <div id="f_metro_id"> 
    <br>
